...
set /p Workspace=Enter path to healthypi-move-workspace: 
set ZEPHYR_BASE=%Workspace%\zephyr
...
%toolchain_INSTALL_DIR%\opt\zephyr-sdk\riscv64-zephyr-elf\bin;^
%toolchain_INSTALL_DIR%\opt\zephyr-sdk;^
%ZEPHYR_BASE%;%PATH%
...
if exist "%VERSION_FILE%" (
    ...
    echo Zephyr-Workspace-SDK Version: !ZEPHYR_SDK_VERSIO!
) else (
...
REM Check if west exists
west --version >nul 2>&1
IF %ERRORLEVEL% NEQ 0 (
...
pushd "%Workspace%" || (
    echo ERROR: Failed to change directory to healthypi-move-workspace!
    pause
    exit /b 1
)
